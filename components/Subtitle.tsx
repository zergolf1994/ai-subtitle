...
import { IUploadResponseData, SubtitleNodeData } from '@/lib/types';
...
export default function Subtitle({
  ...
  const [cues, setCues] = useState<SubtitleNodeData[]>(nodes.map((i): SubtitleNodeData => i.data as SubtitleNodeData));
  const [loading, setLoading] = useState<boolean>(false);
  const [finishNodes, setFinishNodes] = useState<SubtitleNodeData[]>([]);
  const handleTranslate = async (node: SubtitleNodeData) => {
    setLoading(true);
    ...
    if (res.status === 200) {
      const newNode: SubtitleNodeData = (await res.json());

      console.log(`-----------------`)
      console.log(`${newNode.index + 1} / ${length}`)
      console.log(`${newNode.originalText}\n${newNode.targetText}`)
  
      const newCues = [...cues];
      newCues[newNode.index].targetText = newNode.targetText;
      setCues(newCues);
      setFinishNodes([...finishNodes, newNode]);
      setLoading(false);
    } else {
      setLoading(false);
  ...
  const handleRunAll = async () => {
    ...
    for (let i = 0; i < cues.length; i++) {
      const cue = cues[i];
      if (!cue.targetText) {
        await handleTranslate(cue);
      }
    }
  }

  const isLoading = (key: string) => {
    return loading && finishNodes.some((i) => i.key !== key);
  }
    ...
    <div>
      ...
      <div className="overflow-y-auto h-96 p-4">
        <ol className="relative border-l border-gray-200 dark:border-gray-700">
          {cues.map((cue, index) => {
            const { start, end, originalText, targetText, key } = cue;
            return (
                ...
                <div className="p-2 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-700 dark:border-gray-600">
                  <div className="items-center justify-between sm:flex">
                    <time className="mb-1 text-xs font-normal text-gray-400 sm:order-last sm:mb-0">
                      {
                        !isLoading(key) ? (
                          <button type="button" className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-transparent rounded-lg hover:bg-gray-900 hover:text-white focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-gray-900 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:bg-gray-700">
                            <svg className="w-3 h-3 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 20">
