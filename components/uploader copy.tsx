...
export default function Uploader() {
  ...
  const [subtitle, setSubtitle] = useState<IUploadResponseData>();
  ...
  const uploadPictureHandler = async (file: any) => {
    ...
    try {
      ...
      if (res.status === 200) {
        const { data: { nodes, filename, length } }: IUploadResponseData = (await res.json());
        console.log('nodes', nodes);
        setSubtitle({ nodes, filename, length })
      } else {
    ...
    >
      <div>
        ...
        >
          ...
          {JSON.stringify(subtitle?.nodes)}
          <div>字幕数量: {subtitle?.length}</div>
          <div>字幕文件名: {subtitle?.filename}</div>
        </label>
